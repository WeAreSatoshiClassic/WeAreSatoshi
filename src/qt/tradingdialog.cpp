    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->ICBAvailableLabel->setPalette(sample_palette);
...
void tradingDialog::UpdaterFunction(){
    //Get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::BuyICB(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::SellICB(QString OrderType, double Quantity, double Rate){
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       QString balance = GetBalance(TRADING_THIS_COIN);
                                       QString str;
                                       QJsonObject ResultObject =  GetResultObjectFromJSONObject(balance);

                                       ui->ICBAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("BTC");
                         ...
                         Response = GetBalance(TRADING_THIS_COIN);

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->ICBBalanceLabel,*ui->ICBAvailableLabel2,*ui->ICBPendingLabel, QString::fromUtf8(TRADING_THIS_COIN),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputICB->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::on_UpdateKeys_clicked()
{
  ...
  if ( ResponseObject.value("success").toBool() == false){
       QMessageBox::information(this,"API Configuration Failed","API configuration was unsuccesful.\nNote: If you have just created your API key at Bittrex it can take a few minutes for it to start working.");
...
{
    //calculate amount of BTC that can be gained from selling ICB available balance
    QString responseA = GetBalance(TRADING_THIS_COIN);
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableICB = ResultObject["Available"].toDouble();

    ui->UnitsInputICB->setText(str.number(AvailableICB,'i',8));
}
...
void tradingDialog::on_BuyICB_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyICB(Order,Quantity,Rate);
...
void tradingDialog::on_SellICBBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputICB->text().toDouble();
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellICB(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputICB_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
